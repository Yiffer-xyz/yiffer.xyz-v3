...
export const action: ActionFunction = async ({ request, context }) => {
  ...
  const response = await fetch(`${context.URL_BASE_V2}/upload`, {
    method: 'POST',
...
async function getArtists(urlBase: string): Promise<Artist[]> {
  const artistsResponse = await fetch(`${urlBase}/artists?includePending=true`);
...
async function uploadFiles(
  ...
  const uploadPromises = filesFormDatas.map(uploadFormData =>
    fetch(`${uploadUrlBase}/upload-pages`, {
      method: 'POST',
      body: uploadFormData,
    })
  );
