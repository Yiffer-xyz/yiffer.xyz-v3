import type { Route } from './+types/assign-action';
import { queryDb, queryDbExec } from '~/utils/database-facade';
import { parseFormJson } from '~/utils/formdata-parser';
import { type ResultOrErrorPromise, noGetRoute } from '~/utils/request-helpers';
import {
  create400Json,
  createSuccessJson,
  makeDbErrObj,
...
async function assignActionToMod(
  ...
  const existingModIdRes = await queryDb<any[]>(db, getExistingModId, [actionId]);
