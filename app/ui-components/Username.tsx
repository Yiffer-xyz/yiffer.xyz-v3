...
export default function Username({
  ...
  const innerContent = useMemo(
    () => (
      <>
        ...
        <div className="flex flex-col justify-evenly h-[64px]">
          ...
          {user ? (
            <p className="text-sm">Member for {formatDistanceToNow(user.createdTime)}</p>
          ) : (
            <p className="text-sm pr-12">Member for...</p>
          )}
        </div>
    ...
    [user, username, hasAnyBadges, pagesPath]
  );
      ...
      {isTouchDevice ? (
        <>
          <Button
            text={username}
            variant="naked"
            style={{ marginTop: 0, padding: 0, fontSize: 16 }}
          />
        </>
      ) : (
        <Link
          href={overrideLink ?? `/user/${id}`}
          text={username}
          showRightArrow
          isInsideParagraph
        />
      ...
      {isHovering && (
        <RemixLink
          ...
          className={`flex absolute bg-white hover:bg-blue-strong-900 dark:bg-gray-100 dark:hover:bg-[#1a1b20]
                      ...
                      shadow-md dark:shadow-lg flex-row gap-2 items-center cursor-pointer`}
        >
          {innerContent}
        </RemixLink>
      )}
