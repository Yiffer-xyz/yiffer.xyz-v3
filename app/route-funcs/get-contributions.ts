...
export async function getContributions(
  db: D1Database,
  userId: number
): ResultOrErrorPromise<Contribution[]> {
  const dbStatements: QueryWithParams[] = [
    {
      query: yourContributedComicsQuery,
      params: [userId],
      queryName: 'Contributions, comics',
    },
    {
      query: yourTagSuggestionsQuery,
      params: [userId],
      queryName: 'Contributions, tag suggestions',
    },
    {
      query: yourComicProblemsQuery,
      params: [userId],
      queryName: 'Contributions, comic problems',
    },
    {
      query: yourComicSuggestionsQuery,
      params: [userId],
      queryName: 'Contributions, comic suggestions',
    },
  ];
  ...
  if (dbRes.isError) {
    return makeDbErrObj(dbRes, 'Error getting contributions', { userId });
  }
